...
class OpenAPIConverter(FieldConverterMixin):
    ...
    def init_parameter_attribute_functions(self):
        self.parameter_attribute_functions = [
            self.field2required,
            self.list2param,
        ]

    def add_parameter_attribute_function(self, func):
        """Method to add a field parameter function to the list of field
        parameter functions that will be called on a field to convert it
        and its property
        to a field parameter.
